...
(function (root) {
  ...
  function settingsMarkup() {
    return [
      ...
      '<div class="rounded-md border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-800/80 p-2.5">',
      '  <div class="flex items-center justify-between gap-2">',
      '    <span class="text-xs font-semibold text-slate-700 dark:text-slate-300">LM Studio 설치 모델</span>',
      '    <span id="settings-lmstudio-installed-state" class="px-2 py-0.5 rounded-full bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-[10px]">확인 전</span>',
      '  </div>',
      '  <div id="settings-lmstudio-installed-models" class="mt-2 max-h-32 overflow-y-auto rounded border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-900 px-2 py-2 text-xs text-slate-700 dark:text-slate-200" aria-live="polite">확인 전</div>',
      '  <p id="settings-lmstudio-installed-models-detail" class="mt-1.5 text-[11px] text-slate-500 dark:text-slate-400">LM Studio에 다운로드되어 있는 모델을 표시합니다.</p>',
      '</div>',
      '<div class="rounded-md border border-amber-200 dark:border-amber-900/70 bg-amber-50/70 dark:bg-amber-950/20 p-2.5">',
      '  <div class="text-xs font-semibold text-slate-700 dark:text-slate-300">LM Studio 서버 옵션</div>',
      '  <div class="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-2">',
      '    <label class="flex items-center gap-2 rounded border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-2.5 py-2 text-xs text-slate-700 dark:text-slate-200">',
      '      <input id="settings-lmstudio-serve-network" type="checkbox" checked class="rounded border-slate-300 dark:border-slate-600">',
      '      <span>Serve on Local Network 켜기</span>',
      '    </label>',
      '    <label class="flex items-center gap-2 rounded border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-2.5 py-2 text-xs text-slate-700 dark:text-slate-200">',
      '      <input id="settings-lmstudio-enable-cors" type="checkbox" checked class="rounded border-slate-300 dark:border-slate-600">',
      '      <span>Enable CORS 켜기</span>',
      '    </label>',
      '  </div>',
      '  <button type="button" id="settings-lmstudio-apply-server-options" onclick="applySettingsLMStudioServerOptions()" class="mt-2 px-2 py-1.5 rounded bg-amber-600 text-white text-xs hover:bg-amber-700">선택한 서버 옵션 적용</button>',
      '  <p class="mt-1.5 text-[10px] leading-relaxed text-amber-700 dark:text-amber-300">적용하면 LM Studio 서버가 잠시 재시작됩니다. 로컬 네트워크 공개와 CORS는 신뢰할 수 있는 네트워크에서만 사용하고, 외부 접근 시 인증을 함께 설정하세요.</p>',
      '</div>',
      ...
      '    </label>',
      '    <label class="text-xs text-slate-600 dark:text-slate-400">FAST MAX TOKEN',
      '      <input id="settings-aichat-fast-max-tokens" type="number" min="1" step="1" value="3000" class="mt-1 w-full px-2 py-1.5 border rounded bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-600">',
      '    </label>',
      '    <label class="text-xs text-slate-600 dark:text-slate-400">LIMIT TIME (초)',
      '      <input id="settings-aichat-fast-timeout" type="number" min="1" step="1" value="120" class="mt-1 w-full px-2 py-1.5 border rounded bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-600">',
      '    </label>',
      '  </div>',
      '  <label class="mt-2 flex items-center gap-2 text-xs text-slate-600 dark:text-slate-400">',
      '    <input id="settings-aichat-fast-safety-timeout" type="checkbox" checked class="rounded border-slate-300 dark:border-slate-600">',
      '    <span>안전 옵션 · 접속이 오래 걸리면 제한 시간을 자동으로 120초까지 확보</span>',
      '  </label>',
      '  <label class="mt-2 flex items-center gap-2 text-xs text-slate-600 dark:text-slate-400">',
      '    <input id="settings-aichat-fast-complete-streaming" type="checkbox" checked class="rounded border-slate-300 dark:border-slate-600">',
      '    <span>답변 완료 보장 · 답변이 시작되면 제한 시간이 지나도 완료될 때까지 유지</span>',
      '  </label>',
      ...
      '  <button type="button" onclick="loadSettingsLMStudioInstalledModels()" class="px-2 py-1.5 rounded border border-slate-300 dark:border-slate-600 text-xs">설치 모델 목록 불러오기</button>',
