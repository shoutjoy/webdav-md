(function () {
    ...
    function getSidebarShellHtml() {
        return [
            ...
            '    <button type="button" id="btn-github-sync" onclick="pullGithubRepo()" class="hidden h-9 w-9 shrink-0 items-center justify-center bg-indigo-600 hover:bg-indigo-700 rounded-md text-white transition-colors" title="GitHub 저장소 Pull 동기화" aria-label="GitHub 저장소 Pull 동기화"><i data-lucide="refresh-cw" class="w-4 h-4"></i></button>',
            '    <button type="button" id="btn-clear-unused-cache" onclick="clearUnusedCache()" class="flex h-9 w-9 shrink-0 items-center justify-center bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 rounded-md text-slate-700 dark:text-slate-200 transition-colors" title="임시 캐시 지우기" aria-label="임시 캐시 지우기"><i data-lucide="refresh-ccw" class="w-4 h-4"></i></button>',
            '  </div>',
            '</div>'
        ].join('');
    }
