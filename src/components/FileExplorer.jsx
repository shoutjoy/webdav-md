...
import { Archive, Check, ChevronDown, ChevronRight, Copy, Download, Edit, Eye, FileInput, FilePlus2, FileText, Folder, FolderInput, FolderOpen, FolderPlus, PanelLeftClose, PanelLeftOpen, Trash2, Upload, X } from 'lucide-react';
import MoveDestinationModal from './MoveDestinationModal.jsx';

function TreeItem({ item, depth, expandedPaths, selectedFolderPath, selectionMode, checkedPaths, loading, editorLoading, copiedKey, formatBytes, onToggleFolder, onToggleExpanded, onToggleChecked, onCopyUrl, onOpenFile, onDownload, onRename, onMove, onDelete }) {
  ...
  return <>
    <div className={`group flex min-h-9 items-center border-b border-slate-100 pr-2 text-sm transition dark:border-slate-800 ${item.isDirectory && selectedFolderPath === item.remotePath ? 'bg-indigo-50 text-indigo-800 ring-1 ring-inset ring-indigo-300 dark:bg-indigo-950/50 dark:text-indigo-100 dark:ring-indigo-700' : 'hover:bg-slate-50 dark:hover:bg-slate-800/80'}`} style={{ paddingLeft: `${8 + depth * 18}px` }} role="treeitem" aria-expanded={expandable ? expanded : undefined} aria-selected={item.isDirectory ? selectedFolderPath === item.remotePath : undefined}>
      ...
      <div className="ml-1 hidden shrink-0 items-center gap-0.5 group-hover:flex group-focus-within:flex">
        ...
        {!item.isArchiveEntry && <button type="button" onClick={() => onRename(item.name)} disabled={loading} className="rounded p-1 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700" title="이름 변경"><Edit size={14}/></button>}
